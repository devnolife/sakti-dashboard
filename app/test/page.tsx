...
// NEW: Type definitions for dynamic Excel-driven data
interface StudentRowRaw {
  [key: string]: any;
  certificateTitle?: string; // NEW optional title for front page
  name?: string;
  program?: string;
  subtitle?: string;
  issueDate?: string;
  ...
  assignmentCompletion?: number | string;
  participationScore?: number | string;
  overallGrade?: string;
  gradesBreakdown?: string; // JSON string
  competencies?: string; // JSON string
  weeklyData?: string; // JSON array string
  technologies?: string; // comma separated
  instructorFeedback?: string;
  futureRecommendations?: string; // comma separated
}

// FIX: proper alias to infer structure from defaultStudentData
...
function mapRowToStudent(row: StudentRowRaw): StudentDataType {
  const gradesBreakdown = safeParseJSON(row.gradesBreakdown, defaultStudentData.grades.breakdown);
  const competencies = safeParseJSON(row.competencies, defaultStudentData.competencies);
  const weeklyData = safeParseJSON(row.weeklyData, defaultStudentData.learningTime.weeklyData);
  const technologies = typeof row.technologies === 'string' ? row.technologies.split(/[,;]/).map(s => s.trim()).filter(Boolean) : defaultStudentData.technologies;
  const futureRecommendations = typeof row.futureRecommendations === 'string' ? row.futureRecommendations.split(/[,;]/).map(s => s.trim()).filter(Boolean) : defaultStudentData.futureRecommendations;

  return {
    ...defaultStudentData,
    certificateTitle: row.certificateTitle || defaultStudentData.certificateTitle,
    ...
    issueDate: row.issueDate || defaultStudentData.issueDate,
    verificationId: row.verificationId || defaultStudentData.verificationId,
    stats: {
      ...
      attendanceRate: toNumber(row.attendanceRate, defaultStudentData.stats.attendanceRate),
      assignmentCompletion: toNumber(row.assignmentCompletion, defaultStudentData.stats.assignmentCompletion),
      participationScore: toNumber(row.participationScore, defaultStudentData.stats.participationScore)
    },
    grades: {
    ...
    learningTime: {
      ...
      weeklyData: Array.isArray(weeklyData) ? weeklyData : defaultStudentData.learningTime.weeklyData,
      total: `${defaultStudentData.learningTime.hours}h ${defaultStudentData.learningTime.minutes}m`
    },
    competencies: competencies as any,
    technologies,
    instructorFeedback: row.instructorFeedback || defaultStudentData.instructorFeedback,
    futureRecommendations
  } as StudentDataType;
}

interface CleanCircularProgressProps { percentage?: number; size?: number; strokeWidth?: number; }
  ...
  return (
    <div className="relative flex items-center justify-center" style={{ width: size, height: size }}>
      <svg width={size} height={size} className="transform -rotate-90">
        ...
        <circle cx={size/2} cy={size/2} r={radius} stroke="url(#progressGradient)" strokeWidth={strokeWidth} fill="none" strokeDasharray={circumference} strokeDashoffset={offset} strokeLinecap="round" className="transition-all duration-1000 ease-out" />
        <defs>
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        <span className={`text-2xl font-black bg-gradient-to-r from-emerald-600 to-blue-600 bg-clip-text text-transparent ${montserrat.className}`}>{Math.round((size/110)*231)}h</span>
        <span className={`text-xs text-gray-600 font-medium ${poppins.className}`}>Total</span>
      </div>
    </div>
  );
};
...
const Sparkline = ({ data, color = "#10b981" }: SparklineProps) => {
  if (!Array.isArray(data) || data.length === 0) return null; const max = Math.max(...data); const min = Math.min(...data); const range = max - min || 1;
  const points = data.map((v,i)=>{ const x=(i/(data.length-1))*100; const y=100-((v-min)/range)*100; return `${x},${y}`; }).join(' ');
  return (
    <div className="relative w-16 h-8">
      <svg width="100%" height="100%" viewBox="0 0 100 100" className="overflow-visible">
        <polyline points={points} fill="none" stroke={color} strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" className="transition-all duration-300" />
        {data.map((v,i)=>{ const x=(i/(data.length-1))*100; const y=100-((v-min)/range)*100; return <circle key={i} cx={x} cy={y} r="2" fill={color} className="animate-pulse" style={{ animationDelay: `${i*0.1}s` }} />; })}
      </svg>
    </div>
  );
};

interface GradeIndicatorProps { grade: string; score: number; }
const GradeIndicator = ({ grade, score }: GradeIndicatorProps) => {
  const colors: Record<string,string> = { 'A+':'bg-green-500','A':'bg-green-400','A-':'bg-lime-400','B+':'bg-yellow-400','B':'bg-yellow-300','B-':'bg-orange-400','C+':'bg-orange-300','C':'bg-red-400' };
  return (
    <div className="flex items-center gap-2">
      <div className={`w-8 h-8 ${colors[grade]||'bg-gray-400'} rounded-full flex items-center justify-center text-white font-bold text-sm`}>{grade}</div>
      <span className="text-xs text-gray-600">{score}%</span>
    </div>
  );
};

function CertificateFront({ studentData, montserrat, poppins, openSans }: any) {
  return (
...
function CertificateBack({ studentData, montserrat, poppins, openSans }: any) {
  return (
      ...
      <div className="relative z-10 mb-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex gap-2">
            {studentData.technologies.map((tech: string, index: number) => (
              <div key={index} className={`px-3 py-1 text-sm font-medium rounded-full border ${getRandomBadgeColor()} ${poppins.className} transition-all duration-200 hover:scale-105`}>{tech}</div>
            ))}
          </div>
          <div className="flex items-center gap-2">
            <span className={`text-sm text-gray-600 ${poppins.className}`}>Nilai Keseluruhan:</span>
            <div className="flex items-center justify-center w-6 h-6 text-sm font-bold text-white bg-green-500 rounded-full">{studentData.grades.overall}</div>
          </div>
        </div>
      </div>
      ...
      <div className="relative z-10 grid flex-grow grid-cols-12 gap-5">
        ...
        <div className="col-span-7">
          ...
          <div className="space-y-2">
            {studentData.competencies.map((competency: any, index: number) => (
              <div key={index} className="group relative hover:scale-[1.01] transition-all duration-300">
                <div className="flex items-center justify-between mb-1">
                  <div className="flex items-center gap-2">
                    <div className="w-2 h-2 rounded-full" style={{ background: `linear-gradient(90deg, ${competency.startColor} 0%, ${competency.endColor} 100%)` }} />
                    <span className={`text-xs font-medium text-gray-800 ${poppins.className}`}>
                      {competency.name}
                    </span>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className={`text-xs px-2 py-0.5 rounded-full ${competency.level === 'Expert' ? 'bg-green-100 text-green-700' : competency.level === 'Advanced' ? 'bg-blue-100 text-blue-700' : competency.level === 'Intermediate' ? 'bg-yellow-100 text-yellow-700' : 'bg-gray-100 text-gray-700'} ${poppins.className}`}>{competency.level}</span>
                    <span className={`text-xs font-bold text-gray-900 ${montserrat.className}`}>{competency.value}%</span>
                  </div>
                </div>
                <div className="relative h-2 overflow-hidden bg-gray-200 rounded-full">
                  <div className="h-full transition-all duration-1000 ease-out rounded-full" style={{ background: `linear-gradient(90deg, ${competency.startColor} 0%, ${competency.endColor} 100%)`, width: `${competency.value}%` }} />
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="mt-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              {studentData.grades.breakdown.map((grade: any, index: number) => (
        ...
        <div className="col-span-2">
          ...
          <div>
            ...
            <div className="space-y-2">
              {studentData.futureRecommendations.map((rec: string, index: number) => (
                <div key={index} className={`text-sm p-2 bg-blue-50 rounded text-blue-700 ${poppins.className}`}>
                  {rec === 'Advanced Microservices' ? 'Microservices Lanjutan' : rec === 'Cloud Architecture' ? 'Arsitektur Cloud' : rec === 'DevOps Practices' ? 'Praktik DevOps' : rec}
                </div>
              ))}
            </div>
          </div>
...
export default function CertificatePage() {
  ...
  // NEW state for excel records
  const [records, setRecords] = useState<StudentDataType[]>([]);
  const [selectedIndex, setSelectedIndex] = useState<number>(0);
  const activeStudent = records[selectedIndex];
  const studentData = activeStudent || defaultStudentData; // reuse existing references below
  ...
  const handleExcelUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (evt) => {
      ...
      const json: StudentRowRaw[] = XLSX.utils.sheet_to_json(sheet, { defval: '' });
      const mapped = json.map(mapRowToStudent);
      if (mapped.length) {
        setRecords(mapped);
        setSelectedIndex(0);
      }
  ...
  const handleDownloadTemplate = () => {
    const headers = [
      'certificateTitle','name','program','subtitle','issueDate','verificationId','meetings','totalScore','materials','attendanceRate','assignmentCompletion','participationScore','overallGrade','gradesBreakdown','competencies','weeklyData','technologies','instructorFeedback','futureRecommendations'
    ];
    const example = [{
      certificateTitle: 'Backend Developer I',
      name: 'Nama Mahasiswa',
  ...
  return (
    <div className="flex flex-col items-center justify-start min-h-screen p-4 bg-gray-50 gap-4">
      {/* NEW: Upload Section */}
      <div className="w-full max-w-5xl p-4 bg-white border rounded-lg shadow-sm print:hidden">
        <h2 className="mb-2 text-lg font-semibold">Upload Data Sertifikat (Excel)</h2>
        <p className="mb-4 text-sm text-gray-600">Isi kolom sesuai kebutuhan. Kolom wajib minimal: name, program, issueDate, verificationId. Kolom lain opsional. Untuk field kompleks gunakan format JSON (misal competencies, gradesBreakdown, weeklyData). Gunakan tombol Template untuk contoh struktur.</p>
        <div className="flex flex-wrap items-center gap-3">
          <input type="file" accept=".xlsx,.xls" onChange={handleExcelUpload} className="text-sm" />
          <button onClick={handleDownloadTemplate} className="px-3 py-1 text-sm text-white bg-blue-600 rounded hover:bg-blue-700">Download Template</button>
          {records.length > 0 && <span className="text-sm text-gray-700">Total Record: {records.length}</span>}
        </div>
        {records.length > 0 && (
          <div className="mt-4 overflow-auto max-h-56 border rounded">
            <table className="w-full text-sm">
              <thead className="bg-gray-100">
                <tr>
                  ...
                  <th className="p-2 text-left">Kehadiran</th>
                  <th className="p-2 text-left">Aksi</th>
                </tr>
              </thead>
              <tbody>
                {records.map((r, i) => (
                  <tr key={i} className={"border-t " + (i === selectedIndex ? 'bg-blue-50' : '')}>
                    <td className="p-2">{i + 1}</td>
                    <td className="p-2 whitespace-nowrap">{r.name}</td>
                    <td className="p-2 whitespace-nowrap">{r.program}</td>
                    <td className="p-2">{r.grades.overall}</td>
                    <td className="p-2">{r.stats.attendanceRate}%</td>
                    <td className="p-2"><button onClick={() => { setSelectedIndex(i); setShowBack(false); }} className="px-2 py-1 text-xs text-white bg-emerald-600 rounded hover:bg-emerald-700">Pilih</button></td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {/* A4 Landscape Certificate Container (on-screen toggling) */}
      <div className="overflow-hidden bg-white rounded-lg shadow-2xl a4-landscape print:hidden">
        <div className="relative certificate-content">
          ...
          {showBack ? <CertificateBack studentData={studentData} montserrat={montserrat} poppins={poppins} openSans={openSans} /> : <CertificateFront studentData={studentData} montserrat={montserrat} poppins={poppins} openSans={openSans} />}
        </div>
      </div>

      {/* PRINT VERSION: always render both pages */}
      <div className="hidden print:block">
        <div className="overflow-hidden bg-white rounded-lg a4-landscape mb-4">
          <div className="relative certificate-content">
            <div className="absolute border-4 border-black pointer-events-none inset-6 rounded-3xl"></div>
            <CertificateFront studentData={studentData} montserrat={montserrat} poppins={poppins} openSans={openSans} />
          </div>
        </div>
        <div className="overflow-hidden bg-white rounded-lg a4-landscape">
          <div className="relative certificate-content">
            <div className="absolute border-4 border-black pointer-events-none inset-6 rounded-3xl"></div>
            <CertificateBack studentData={studentData} montserrat={montserrat} poppins={poppins} openSans={openSans} />
          </div>
        </div>
      </div>
      ...
      {/* A4 Print Styles */}
      <style jsx global>{`
        .a4-landscape { width: 297mm; height: 210mm; max-width: 297mm; max-height: 210mm; margin: 0 auto; position: relative; box-sizing: border-box; }
        ...
        @media print { body { margin:0; padding:0; background:white !important; } .a4-landscape { page-break-after: always; } .a4-landscape:last-child { page-break-after: auto; } .certificate-content { padding:8mm; } .print\\:hidden { display:none !important; } }
        @media screen { .a4-landscape { transform:scale(0.8); transform-origin:center; } }
        @media screen and (max-width:1400px){ .a4-landscape { transform:scale(0.6);} }
        @media screen and (max-width:1000px){ .a4-landscape { transform:scale(0.5);} }
        @keyframes flowEffect {0%{transform:translateX(-100%) skewX(-12deg);}50%{transform:translateX(0%) skewX(-12deg);}100%{transform:translateX(100%) skewX(-12deg);} }
