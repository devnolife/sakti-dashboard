...
const FeaturesSection = () => {
  ...
  return (
    <section id="features" className="relative py-12 md:py-16 lg:py-20 overflow-hidden bg-gradient-to-br from-purple-50/30 via-white to-cyan-50/30">
      ...
      <div className="relative px-4 mx-auto max-w-7xl sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-10 md:mb-14">
          <div className="inline-flex items-center mb-4 md:mb-6">
            <Badge className="px-4 md:px-5 py-1.5 md:py-2 text-xs md:text-sm text-white rounded-full shadow-lg bg-cyan-600 shadow-cyan-500/20">
              <Globe className="w-3 h-3 md:w-3.5 md:h-3.5 mr-2" />
              Fitur Unggulan
            </Badge>
          </div>

          <h2 className="mb-4 md:mb-5 text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold leading-tight text-gray-900">
            Solusi Digital <span className="text-cyan-600">Terpadu</span> untuk Fakultas Teknik
          </h2>

          <p className="max-w-3xl mx-auto text-sm md:text-base lg:text-lg leading-relaxed text-gray-600 px-4">
        ...
        <div className="space-y-4 md:space-y-6 overflow-hidden">
          ...
          <div className="relative">
            <div className="flex gap-4 md:gap-5 animate-scroll-ltr">
              {[...features.slice(0, 5), ...features.slice(0, 5)].map((feature, index) => {
                const colors = getColorClasses(feature.color, index % features.length)
                return (
                  <Card
                    key={`row1-${index}`}
                    className={`group relative p-4 md:p-5 border-2 border-gray-200 rounded-2xl bg-white shadow-lg transition-all duration-500 hover:-translate-y-3 hover:shadow-2xl ${colors.cardHover} overflow-hidden flex-shrink-0 w-[240px] md:w-[280px]`}
                  >
                    ...
                    <div className="relative mb-3 md:mb-4">
                      <div className={`inline-flex items-center justify-center w-10 h-10 md:w-12 md:h-12 rounded-xl ${colors.iconBg} transition-all duration-500 group-hover:scale-110 group-hover:rotate-6 shadow-lg`}>
                        <feature.icon className={`w-5 h-5 md:w-6 md:h-6 ${colors.iconColor}`} />
                      </div>
                      {/* Icon shadow */}
                      <div className={`absolute top-0 left-0 w-10 h-10 md:w-12 md:h-12 rounded-xl ${colors.iconBg} blur-xl opacity-0 group-hover:opacity-50 transition-opacity duration-500`} />
                    </div>

                    {/* Content */}
                    <div className="relative space-y-1.5 md:space-y-2">
                      <h3 className="text-base md:text-lg font-bold text-gray-900 transition-colors duration-300 group-hover:text-primary">
                        {feature.title}
                      </h3>
          ...
          <div className="relative">
            <div className="flex gap-4 md:gap-5 animate-scroll-rtl">
        ...
        <div className="mt-12 md:mt-16 text-center px-4">
          <div className="inline-flex items-center gap-2 px-4 md:px-7 py-2.5 md:py-3.5 rounded-full text-xs md:text-sm font-semibold bg-purple-50 text-purple-700 border-2 border-purple-200">
            <Shield className="w-4 h-4 md:w-5 md:h-5 text-purple-600" />
            <span className="text-center">Trusted by 5000+ Students & 180+ Faculty Members</span>
          </div>
        </div>
      </div>
    </section>
